...
def computeCost(w, X, y):
    ...
    sigmoidFn = sigmoid(np.dot(X, w.T))
    minCost_J = - (float(1) /noOfSamples) * ((y * np.log(sigmoidFn)) + ((1 - y) * np.log(1 - sigmoidFn)))
    return minCost_J
...
def dl1(vec):
    ...
    # grad = np.empty(np.shape(1))
    grad = np.array(np.sign(vec))
    # grad =
    # print ("Type of grad is", type(grad))
    # print ("Vec is", vec)
    # print ("Type of vec is", type(vec))
    # print ("abs is", abs(vec) < 1e-4)
